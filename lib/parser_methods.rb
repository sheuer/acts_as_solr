module ActsAsSolr #:nodoc:
  ...
  module ParserMethods
    ...
    def reorder(things, ids)
      ...
      raise "Out of sync! Found #{ids.size} items in index, but only #{things.size} were found in database! The following ids were missing: #{(ids - things.collect{|x| x.id}).join(',')}" unless things.size == ids.size
